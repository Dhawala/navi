...
@section('content')
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-4 text-gray-800">Allocations</h1>
                ...
                <div class="card-body">
                    <div class="form-group">
                        <div class="col-md-4">
                            ...
                            <select disabled class="form-control" name="emp_no" id="emp_no" data-live-search="true">
                                @foreach($lecturers as $lecturer)
                                    <option value="{{$lecturer->emp_no}}" {{( $lecturer->emp_no==$allocation->emp_no?'selected':'') }}>{{$lecturer->emp_no}}-{{$lecturer->name}}</option>
                                @endforeach
                            </select>
                            <label>Schedule id</label>
                            <select disabled class="form-control" name="schedule_id" id="schedule_id" data-live-search="true">
